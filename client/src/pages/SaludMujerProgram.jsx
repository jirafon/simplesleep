...
function SaludMujerProgram() {
  ...
  const content = useMemo(() => getProgramContent('mujer'), []);
  const { addToCart, isInCart } = useCart();

  const packs = useMemo(() => {
    return getProgramPacks('mujer');
  }, []);
  ...
  const handleAddPackToCart = (pack) => {
    const cartItemId = `program-pack-mujer-${pack.id}`;
                  ...
                  <div className="p-5 md:p-6 border-b border-slate-100">
                    ...
                    <div className="mt-4 flex flex-wrap gap-3">
                      <button
                        ...
                        disabled={isInCart(`program-pack-mujer-${pack.id}`)}
                        className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition ${
                          isInCart(`program-pack-mujer-${pack.id}`)
                            ? 'bg-slate-200 text-slate-500 cursor-not-allowed'
                            : 'bg-slate-900 text-white hover:bg-black'
                        }`}
                      >
                        {isInCart(`program-pack-mujer-${pack.id}`) ? 'En el carrito' : 'Agregar pack al carrito'}
                      </button>
...
export default SaludMujerProgram;
